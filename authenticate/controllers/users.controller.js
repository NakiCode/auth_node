...
exports.addSocial = catchAsync(async (req, res, next)=>{
    ...
    if (!check){return res.status(400).json({message:"Utilisateur introuvable, verifier vos informations !"})}
...
exports.deleteSocial = catchAsync(async (req, res, next)=>{
    ...
    const check = await tbl_User.exists({_id:id})
    if (!check){return res.status(400).json({message:"Utilisateur introuvable, verifier vos informations !"})}
    const social = await tbl_User.findOneAndUpdate({_id:id},{
        $pull : {social: accout}
    })
    if (!social){
...
exports.updateUser = catchAsync(async (req, res, next)=>{
    ...
    const check = await tbl_User.exists({_id:id})
    if (!check){return res.status(400).json({message:"Utilisateur introuvable, verifier vos informations !"})}
    const updateUser = await tbl_User(id,
        req.body
    ,  { new: true, runValidators:true })
    if (!updateUser){return res.status(400).json({message:"Problème de mise à jour ! verifier vos informations"})}
...
exports.getUserInfos = catchAsync(async (req, res, next)=>{
    ...
    if (!getInfos){return res.status(200).json({message:"Problème d'accès aux informations de l'utilisateur verifier vos informations !"})}
...
exports.signIn = catchAsync(async (req, res, next)=>{
    ...
    if(!credation || !password){ return res.status(400).json({message:"Veuillez entrez vos informations de connexion !"})}
    ...
    if (!correct || !findUser){
        res.status(200).json({message:"Vos données de d'authentification sont incorrect !"})
    }
...
exports.restrictToAuth = () =>{
    return async (req, res, next) =>{
        ...
        if (!check.authentifierPhone || !check.authentifierEmail){
            return next(new Error("Vous n'etes pas autorisé à accéder ici Veuillez valide votre email et numéro de telephone !"))
        }
    next()
}}