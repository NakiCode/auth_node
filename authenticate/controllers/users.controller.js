...
exports.addSocial = catchAsync(async (req, res, next) => {
    ...
    if (! check) {
        return res.status(400).json({message: "Utilisateur introuvable, verifier vos informations !"});
    }
    ...
    if (! social) {
        return res.status(400).json({message: "Une erreur d'enregistrement est survenue veuillez verifier vos informations !"});
    }
...
exports.deleteSocial = catchAsync(async (req, res, next) => {
    ...
    if (! social) {
        return res.status(400).json({message: "Une erreur de suppression est survenue veuillez verifier vos informations !"});
    }
...
exports.updateUser = catchAsync(async (req, res, next) => {
    ...
    const updateUser = await tbl_User.findOneAndUpdate({
        _id: id
    }, req.body, {
        new: true,
        runValidators: true
    ...
    if (! updateUser) {
        return res.status(400).json({message: "Problème de mise à jour ! verifier vos informations"});
    }
...
exports.getUserInfos = catchAsync(async (req, res, next) => {
    ...
    const getInfos = await tbl_User.findOne({_id: id}).select("name username email phone social");
    if (! getInfos) {
        return res.status(200).json({message: "Problème d'accès aux informations de l'utilisateur verifier vos informations !"});
    }
...
exports.signIn = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password) {
        return res.status(400).json({message: "Veuillez entrez vos informations de connexion !"});
    }
    const findUser = await tbl_User.findOne({email: email}).select("+password");
    if (! findUser) {
        return res.status(400).json({message: "Vos données d'authentification sont incorrect !"});
    }
    const correct = findUser.correctPassword(password, findUser.password);
    if (! correct || ! findUser) {
        return res.status(200).json({message: "Vos données d'authentification sont incorrect !"});
    }
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (! token) {
        return res.status(500).json({message: "You are not logged in! Please log in to get access !"});
    }
    ...
    if (! freshUser) {
        return next(new Error("The token belonging to this user does no loger exists."));
    }
...
exports.restrictToAuth = catchAsync(async () => {
    return async (req, res, next) => {
        ...
        if (! check.authentifierPhone || ! check.authentifierEmail) {
            return next(new Error("Vous n'etes pas autorisé à accéder ici Veuillez validez votre email et le numéro de telephone !"));
        }
        next();
    };
});
